...
import { getLanguages } from "./gitStats";
...
export function generateImage(data: object) {
  ...
  const topFiveProjects = (data as any).user.repositories.nodes
    .sort(compareStars)
    .slice(0, 5);
  const contributions = (data as any).user.contributionsCollection
    .contributionCalendar.weeks;
  let contributionsProcessed: any = [];

  contributions.forEach((week: any) => {
    week.contributionDays.forEach((day: any) => {
      contributionsProcessed.push({
        date: day.date,
  ...
  topFiveProjects.forEach((project: any, index: number) => {
    let projectName = project.name.substring(0, 18);
    ...
    generateTag(
      ...
      20,
      20 + 25 * index,
      `${projectName} | ${project.stargazerCount} stars`
    );
  });
  ...
  Object.keys(languages.languages).forEach((key: string) => {
    ...
    languagePercentage.push({
      ...
      percentage: Math.round(
        (languages.languages[key] / languages.total) * 100
      ),
    });
  });
